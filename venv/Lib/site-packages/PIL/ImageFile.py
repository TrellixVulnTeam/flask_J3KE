...
def raise_ioerror(error):
    ...
    if not message:
        message = "decoder errors %d" % error
...
class ImageFile(Image.Image):
    ...
    def load(self):
        ...
        self.load_prepare()
        err_code = -3  # initialize to unknown errors
        if not self.map:
            ...
            if e.pushes_fd:
                ...
                while True:
                    ...
                    if s:
                        break
            if s < 0:
                raise IOError("encoder errors %d when writing image file" % s)
            e.cleanup()
    else:
