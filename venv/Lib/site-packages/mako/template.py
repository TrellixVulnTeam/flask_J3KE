...
class Template(object):
    ...
    :param error_handler: Python callable which is called whenever
     ...
     completes. Is used to provide custom errors-rendering
     functions.
    ...
    :param format_exceptions: if ``True``, exceptions which occur during
     ...
     formatted into an HTML errors page, which then becomes the
    ...
    :param include_error_handler: An errors handler that runs when this template
     is included within another one via the ``<%include>`` tag, and raises an
     errors.  Compare to the :paramref:`.Template.error_handler` option.

     .. versionadded:: 1.0.6

     .. seealso::

        :paramref:`.Template.error_handler` - top-level errors handler function
